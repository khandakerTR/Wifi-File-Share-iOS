...
open class TLPhotosPickerViewController: UIViewController {
    ...
    private var singleSelectedMode: Bool {
        get {
            return self.configure.singleSelectedMode
        }
        set {
            self.configure.singleSelectedMode = newValue
        }
    }
...
extension TLPhotosPickerViewController {
    ...
    @objc open func makeUI() {
        ...
        if self.singleSelectedMode {
            multipleSelectionButton.setImage(UIImage.init(named: "multiple_selection_disable"), for: .normal)
            multipleSelectionButton.isHidden = true
        }
        else {
            multipleSelectionButton.setImage(UIImage.init(named: "multiple_selection_enable"), for: .normal)
            multipleSelectionButton.isHidden = false
        }
    }
    ...
    @IBAction open func albumButtonAction() {
          
      }
    
    @IBAction open func dismissButtonAction() {
        self.stopPlay()
        self.dismiss(done: false)
    }
    
    @IBAction open func multipleSelectionButtonAction() {
        if self.singleSelectedMode {
            self.singleSelectedMode = false
            multipleSelectionButton.setImage(UIImage.init(named: "multiple_selection_enable"), for: .normal)
        }
        else {
            self.singleSelectedMode = true
            multipleSelectionButton.setImage(UIImage.init(named: "multiple_selection_disable"), for: .normal)
        }
    }
}
...
extension TLPhotosPickerViewController: UIScrollViewDelegate {
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //self.bottomOptionView.isHidden = scrollView.contentOffset.y  > scrollView.contentSize.height - scrollView.frame.size.height - 100
        
        let isHIdden = scrollView.contentOffset.y  > scrollView.contentSize.height - scrollView.frame.size.height - 100
       self.bottomOptionView.isHidden = isHIdden
    }
}
